...
#define I2C_DP159_BASE XPAR_IIC_0_BASEADDR
...
u8 i2c_dp159_chk(u8 dev) {
	...
	// DP159 ES
	if (dev == DP159_ES) {
		r = XIic_Recv(I2C_DP159_BASE, I2C_DP159_ES_ADDR, (u8 *)buf, 1, XIIC_STOP);
	}
	else
		r = XIic_Recv(I2C_DP159_BASE, I2C_DP159_ZOMBIE_ADDR, (u8 *)buf, 1, XIIC_STOP);
...
u32 i2c_dp159_write(u8 dev, u8 addr, u8 dat)
{
  ...
  // DP159 ES
  if (dev == DP159_ES) {
	  r = XIic_Send(I2C_DP159_BASE, I2C_DP159_ES_ADDR, (u8 *)buf, 2, XIIC_STOP);
  }

  // Zombie
  else {
	  r = XIic_Send(I2C_DP159_BASE, I2C_DP159_ZOMBIE_ADDR, (u8 *)buf, 2, XIIC_STOP);
  }
...
u8 i2c_dp159_read(u8 dev, u8 addr)
{
  ...
  // DP159 ES
  if (dev == DP159_ES) {
	  r = XIic_Send(I2C_DP159_BASE, I2C_DP159_ES_ADDR, (u8 *)buf, 1, XII_REPEATED_START_OPTION);
	  r = XIic_Recv(I2C_DP159_BASE, I2C_DP159_ES_ADDR, (u8 *)buf, 1, XIIC_STOP);
  }

  // Zombie
  else {
	  r = XIic_Send(I2C_DP159_BASE, I2C_DP159_ZOMBIE_ADDR, (u8 *)buf, 1, XII_REPEATED_START_OPTION);
	  r = XIic_Recv(I2C_DP159_BASE, I2C_DP159_ZOMBIE_ADDR, (u8 *)buf, 1, XIIC_STOP);
  }
...
void i2c_dp159_dump(void)
{
  ...
  r = XIic_Recv(I2C_DP159_BASE, I2C_DP159_ES_ADDR, (u8 *)buf, 32, XIIC_STOP);
  for (i = 0; i< 0x20; i++) {
...
#endif
{
  ...
  XIic_WriteReg(I2C_DP159_BASE, XIIC_RESETR_OFFSET,
			XIIC_RESET_MASK);
